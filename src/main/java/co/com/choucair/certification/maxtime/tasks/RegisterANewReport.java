...
public class RegisterANewReport implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                ,WaitUntil.the(INPUT_POPUP,isVisible()).forNoMoreThan(60).seconds()
                ...
                ,WaitUntil.the(A_BUTTON_ACEPT_POPUP,isVisible()).forNoMoreThan(60).seconds()
                ,WaitUntil.the(A_BUTTON_ACEPT_POPUP,isEnabled()).forNoMoreThan(60).seconds()
                ,WaitUntil.the(A_BUTTON_ACEPT_POPUP,isCurrentlyEnabled()).forNoMoreThan(60).seconds()
                ,Click.on(A_BUTTON_ACEPT_POPUP)
                ,WaitUntil.the(TD_ITEMS_LIST,isVisible()).forNoMoreThan(60).seconds()
                ,WaitUntil.the(TD_ITEMS_LIST,isEnabled()).forNoMoreThan(60).seconds()
                ,WaitUntil.the(TD_ITEMS_LIST,isCurrentlyEnabled()).forNoMoreThan(60).seconds()
